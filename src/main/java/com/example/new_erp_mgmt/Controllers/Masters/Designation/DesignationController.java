package com.example.new_erp_mgmt.Controllers.Masters.Designation;
...
public class DesignationController implements Initializable {
    ...
    public TableView<objDesig> tblView;
    ...
    private Stage stage_Desig;
    ObservableList <objDesig> tblData = FXCollections.observableArrayList();
    ...
    private void setCode(){
        ...
        try {
            ...
            ResultSet rs = st.executeQuery("select count(code), max(code) FROM " + db.schema + ".tbl_desig");
            if (rs.next()) {
    ...
    private void setTable(){
        colID.setCellValueFactory(new PropertyValueFactory<objDesig,String>("code"));
        colName.setCellValueFactory(new PropertyValueFactory<objDesig,String>("name"));
        colRemarks.setCellValueFactory(new PropertyValueFactory<objDesig,String>("remarks"));
        colStatus.setCellValueFactory(new PropertyValueFactory<objDesig,String>("status"));
        colSlno.setCellValueFactory(new Callback<TableColumn.CellDataFeatures<objDesig, objDesig>, ObservableValue<objDesig>>() {
            @Override
            public ObservableValue<objDesig> call(TableColumn.CellDataFeatures<objDesig, objDesig> p) {
                return new ReadOnlyObjectWrapper(p.getValue());
            }
        });
        colSlno.setCellFactory(new Callback<TableColumn<objDesig, objDesig>, TableCell<objDesig, objDesig>>() {
            @Override public TableCell<objDesig, objDesig> call(TableColumn<objDesig, objDesig> param) {
                return new TableCell<objDesig, objDesig>() {
                    @Override
                    protected void updateItem(objDesig item, boolean empty) {
    ...
    private void populateTable(){
        ...
        try{
            Statement st = db.con.createStatement();
            ResultSet rs;
            if(chkActive.isSelected()){rs  = st.executeQuery("select * from " + db.schema + ".tbl_desig where status = 1");}
            else{rs = st.executeQuery("select * from " + db.schema + ".tbl_desig ");}
            while(rs.next()){
                String status;
                if(rs.getString("status").equalsIgnoreCase("1")){status ="Active";}else{status="In Active";}
                tblData.add(new objDesig(rs.getString("code"),rs.getString("name"),rs.getString("remarks"),status));
            }
        } catch (Exception e) {
            Alert alt = new Alert(Alert.AlertType.ERROR,e.getMessage(),ButtonType.OK);
            alt.show();
    ...
    private void listener(){
        ...
        txtRemarks.addEventFilter(KeyEvent.KEY_RELEASED, new EventHandler<KeyEvent>() {
            @Override
            public void handle(KeyEvent t) {
                if (t.getCode() == KeyCode.ENTER) {btnSave.requestFocus();}
            }
        });
    ...
    private void fetchData(objDesig obj){
    ...
    private void saveValidation(){
        if(txtID.getText().equalsIgnoreCase("") || txtName.getText().equalsIgnoreCase("") ) {
            Alert alt = new Alert(Alert.AlertType.ERROR, "Please Enter Designation Name!!", ButtonType.OK);
            alt.showAndWait();
            if (alt.getResult() == ButtonType.OK){txtName.requestFocus();}
        }else{
            Alert alt = new Alert(Alert.AlertType.CONFIRMATION, "Are you sure you want to Save this Designation", ButtonType.YES,ButtonType.NO);
            alt.showAndWait();
            if (alt.getResult() == ButtonType.YES){saveData();}else{txtName.requestFocus();}
        }
    }
    private void saveData(){
        try{
            Statement st = db.con.createStatement();
            int rs = st.executeUpdate("insert into "+db.schema+".tbl_desig values(null,'"+txtID.getText()+"','"+txtName.getText().toUpperCase().trim()+"','"+txtRemarks.getText()+"',1," +
                    " '"+ LoginController.user +"','"+dateFormat.format(date)+"',null,null,null,null )");
            if (rs == 1){
                Alert alt = new Alert(Alert.AlertType.ERROR,"Designation Saved Successfully",ButtonType.OK);
                alt.showAndWait();
    ...
    private void updateValidation(){
        if(tblView.getSelectionModel().isEmpty()){
            Alert alt1 = new Alert(Alert.AlertType.ERROR ,"Please Select a Designation from Table",ButtonType.OK);
            alt1.showAndWait();
            ...
            Alert alt2 = new Alert(Alert.AlertType.ERROR, "Please Enter Designation Name!!", ButtonType.OK);
            alt2.showAndWait();
            ...
            Alert alt2 = new Alert(Alert.AlertType.ERROR, "Selected Designation is Already Deleted item. If you want to Update, Please Select Another Brand or ReActivate the Department!!!", ButtonType.OK);
            alt2.showAndWait();
            if (alt2.getResult() == ButtonType.OK) {txtName.requestFocus();}
        } else{
            Alert alt3 = new Alert(Alert.AlertType.CONFIRMATION,"Are you sure you want Update this Designation?",ButtonType.YES,ButtonType.NO);
            alt3.showAndWait();
    ...
    private void updateData(){
        try{
            Statement st = db.con.createStatement();
            int rs = st.executeUpdate("update "+db.schema+".tbl_desig set name='"+txtName.getText().toUpperCase().trim()+"', remarks='"+txtRemarks.getText()+"'," +
                    " updated_by='"+LoginController.user+"', updated_on='"+dateFormat.format(date)+"' where code = '"+txtID.getText()+"'");
            if (rs == 1){
                Alert alt = new Alert(Alert.AlertType.ERROR,"Designation Updated Successfully",ButtonType.OK);
                alt.showAndWait();
                if(alt.getResult() == ButtonType.OK){clear();}
            }
        } catch (Exception e) {
    ...
    private void deleteValidation(){
        if (tblView.getSelectionModel().isEmpty()){
            Alert alt = new Alert(Alert.AlertType.ERROR,"Please Select the Designation for Delete",ButtonType.OK);
            alt.showAndWait();
            if(alt.getResult() == ButtonType.OK){tblView.requestFocus();}
        } else if (tblView.getSelectionModel().getSelectedItem().status == "In Active") {
            Alert alt1 = new Alert(Alert.AlertType.ERROR, "Selected Designation is Already Deleted. Please Select Active Designation ", ButtonType.OK);
            alt1.showAndWait();
            if (alt1.getResult() == ButtonType.OK) {tblView.requestFocus();}
        } else{
            Alert alt2 = new Alert(Alert.AlertType.CONFIRMATION,"Are you sure, You want to delete this Designation?",ButtonType.OK);
            alt2.showAndWait();
    ...
    private void deleteData(){
        try{
            Statement st = db.con.createStatement();
            int rs = st.executeUpdate("update "+db.schema+".tbl_desig set status = 0," +
                    " deleted_by = '"+LoginController.user+"', deleted_on = '"+dateFormat.format(date)+"' where code = '"+txtID.getText()+"'");
            if (rs == 1){
                Alert alt = new Alert(Alert.AlertType.ERROR,"Designation Deleted Successfully",ButtonType.OK);
                alt.showAndWait();
                if(alt.getResult() == ButtonType.OK){clear();}
            }
        } catch (Exception e) {
    ...
    private void activeData(){
        ...
        if (alt.getResult()== ButtonType.YES) {
            try {
                Statement st = db.con.createStatement();
                int rs = st.executeUpdate("update " + db.schema + ".tbl_desig set status= 1 where code ='" + txtID.getText() + "'");
                if (rs == 1) {
                    Alert alt1 = new Alert(Alert.AlertType.CONFIRMATION, "Designation Activated Successfully", ButtonType.OK);
                    alt1.showAndWait();
    ...
    private void CreatePDF(File fileTemp) throws IOException {
        try {
            ...
            Text sub_head = new Text("Designation List").setFontSize(15).setFont(head2);
            ...
            Cell cell = new Cell().add(new Paragraph("Designation ID")).setTextAlignment(TextAlignment.CENTER).setFont(head2);
            table.addHeaderCell(cell);

            cell = new Cell().add(new Paragraph("Designation Name")).setTextAlignment(TextAlignment.CENTER).setFont(head2);
            table.addHeaderCell(cell);
    ...
    public void setStage(Stage stageDesig) {
        this.stage_Desig = stageDesig;
        stage_Desig.setOnCloseRequest(new EventHandler<WindowEvent>() {
            @Override
        ...
        stage_Desig.getScene().addEventFilter(KeyEvent.KEY_PRESSED, new EventHandler<KeyEvent>() {
            @Override
            public void handle(KeyEvent t) {
    ...
    private void setFadeOutTransition() {
        FadeTransition fadeTransition = new FadeTransition(Duration.millis(500), stage_Desig.getScene().getRoot());
